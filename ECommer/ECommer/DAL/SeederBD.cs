...
namespace ECommer.DAL
{
    public class SeederBD
    {
        ...
        private async Task PopulateProductAsync()
        {
            ...
            {
                await AddProductAsync("Medias Grises", "Gray socks", 270000M, 12F, new List<string>() { "Ropa Interior" }, new List<string>() { "calcetines.jpg" });
                await AddProductAsync("PlayStations", "PS5", 300000M, 12F, new List<string>() { "Gamers" }, new List<string>() { "ps5.avif", "PS5GOW.jpg" });
                await AddProductAsync("TV Samsung OLED", "Wonderful TV", 5000000M, 12F, new List<string>() { "Tecnología", "Gamers" }, new List<string>() { "tv.png" });
            }
        }
        ...
        private async Task AddProductAsync(string name, string description, decimal price, float stock, List<string> categories, List<string> images)
        {
            ...
            {
                Guid imageId = await _azureBlobHelper.UploadAzureBlobAsync($"{Environment.CurrentDirectory}\\wwwroot\\images\\product\\{image}", "products");
